...
family_list = ['bala', 'sridhar','revathi','yuvasri','priyan']
for people in family_list:
    print (f"Family members are {people}")
print("next member")
...
c=int(input("get the third number"))
if a==b==c:
    print("a and b, c are same")
elif a>=b and a>=c:
   if a==b:
    ...
    if c==a:
        ...
        print("c and b are same")
    else:
        print(f"c is greater, value of c is {c}")
else:
    print(f"Provide the correct input of {a} and {b} and {c}")
...
popup_click=input("User clicks - Yes/No: ").upper()
if popup_click == "YES":
    print("choose option1: up-coming batches or option2: ASK ANYTHING")
    tabs = input("which option 1/2: ")
    if tabs == "upcoming batch":
        print("You have opted for up-coming batches")
        course_list = input("courses we have DE, GenAI, AzureDE, Devops ")
        if course_list == "DE":
            print('''You have opted for Data Engineering, 
                     Duration - 7 months
                     Date - starting from 10 September, 2025''')
            DE_course_details = input("choose option1: CHECK TIMING or option2: ASK ANYTHING").upper()
            if DE_course_details == "CHECK TIMING":
                print("Available timings is listed")
            if DE_course_details == "ASK ANYTHING":
                print("Redirected to option2-ASK ANYTHING tab again")
        elif course_list == "GenAI":
            print(''' You have opted for Generative AI,
                      Duration - 120 hours,
                      Date - starting july, 2025''')
            GenAI_course_details = input("choose option1: CHECK TIMING or option2: ASK ANYTHING").upper()
            if GenAI_course_details == "CHECK TIMING":
                print("Available timings is listed")
            if GenAI_course_details == "ASK ANYTHING":
                print("Redirected to option2- ASK ANYTHING tab again")
        elif course_list == "AzureDE":
            print('''You have opted for Azure Data Engineering,
                     Duration - 4 months,
                     Date - starting July, 2025''')
            AzureDE_course_details = input("choose option1: CHECK TIMING or option2: ASK ANYTHING").upper()
            if AzureDE_course_details == "CHECK TIMING":
                print("Available timings is listed")
            if AzureDE_course_details == "ASK ANYTHING":
                print("Redirected to option2- ASK ANYTHING tab again")
        elif course_list == "Devops":
            print('''You have opted for Devops,"
                     Duration - 3 months,
                     Date - starting April, 2025''')
            Devops_course_details = input("choose option1: CHECK TIMING or option2: ASK ANYTHING").upper()
            if Devops_course_details == "CHECK TIMING":
                print("Available timings is listed")
            if Devops_course_details == "ASK ANYTHING":
                print("Redirected to option2- ASK ANYTHING tab again")
    elif tabs == "ask anything":
        print("You have opted for ASK ANYTHING")
        description = input("enter your description - ")
        mobile_or_email_id =  input("Enter your email_id/Mobile number - ")
        print(f"ticket raised with the description {description}.",
              "You will receive call from Inceptez within 2 days")
elif popup_click == "NO":
    print("you haven't clicked anything."," Please click the tabs one more time")
...
cart_amt = int(input("Amount purchased for the selected food -"))
if cart_amt >= min_cap_amt:
    offer_amt = cart_amt*discnt
    if offer_amt >= abs(max_cap_amt):
        print(f"the offer amount is {max_cap_amt}")
    elif offer_amt >= max_cap_amt-offer_amt:
        print(f"the offer amount is {offer_amt}")
else :
    print("Ammount still needs to be add in cart - ", min_cap_amt-cart_amt)
...
#Function Based Programming (FBP)
import os
import shutil

path_needed = input("Enter the path: ")
def temp_clean(path_needed:str):
  if os.path.exists(path_needed):
      shutil.rmtree(path_needed)
      print("Path deleted successfully")
  else:
      print("Check path entered are correct")

temp_clean(path_needed)


#Function based programming with arguments
def mail(*args):
     print(type(args))
     if len(args)==3:
         fname= args[0]
         lname= args[1]
         cmpy= args[2]
         full_email = fname + '.' + lname + '@' + cmpy + '.com'
     elif len(args)==2:
         fname= args[0]
         lname= args[1]
         full_email = fname + '.' + lname + '@cpz.com'
     elif len(args)==1:
         fname= args[0]
         full_email = fname + '.example@cpz.com'
     else:
        print("Please provide atleast anyone details")
     return full_email